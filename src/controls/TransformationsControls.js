...
(function () {
	...
	TransformGizmoTranslate = function () {
		...
		lineXGeometry.vertices.push( new Vector3( 0, 0, 0 ), new Vector3( 1, 0, 0 ) );

		var lineYGeometry = new Geometry();
		lineYGeometry.vertices.push( new Vector3( 0, 0, 0 ), new Vector3( 0, 1, 0 ) );

		var lineZGeometry = new Geometry();
		lineZGeometry.vertices.push( new Vector3( 0, 0, 0 ), new Vector3( 0, 0, 1 ) );
		...
		this.pickerGizmos = {
			X: [
				[ new Mesh( new CylinderGeometry( 0.2, 0, 1, 4, 1, false ), new GizmoMaterial( { color: 0xff0000, opacity: 0.25 } ) ), [ 0.6, 0, 0 ], [ 0, 0, -Math.PI / 2 ] ]
			],
			Y: [
				[ new Mesh( new CylinderGeometry( 0.2, 0, 1, 4, 1, false ), new GizmoMaterial( { color: 0x00ff00, opacity: 0.25 } ) ), [ 0, 0.6, 0 ] ]
			],
			Z: [
				[ new Mesh( new CylinderGeometry( 0.2, 0, 1, 4, 1, false ), new GizmoMaterial( { color: 0x0000ff, opacity: 0.25 } ) ), [ 0, 0, 0.6 ], [ Math.PI / 2, 0, 0 ] ]
			],
			XYZ: [
				[ new Mesh( new OctahedronGeometry( 0.2, 0 ), new GizmoMaterial( { color: 0xffffff, opacity: 0.25 } ) ) ]
			],
			XY: [
				[ new Mesh( new PlaneBufferGeometry( 0.4, 0.4 ), new GizmoMaterial( { color: 0xffff00, opacity: 0.25 } ) ), [ 0.2, 0.2, 0 ] ]
			],
			YZ: [
				[ new Mesh( new PlaneBufferGeometry( 0.4, 0.4 ), new GizmoMaterial( { color: 0x00ffff, opacity: 0.25 } ) ), [ 0, 0.2, 0.2 ], [ 0, Math.PI / 2, 0 ] ]
			],
			XZ: [
				[ new Mesh( new PlaneBufferGeometry( 0.4, 0.4 ), new GizmoMaterial( { color: 0xff00ff, opacity: 0.25 } ) ), [ 0.2, 0, 0.2 ], [ -Math.PI / 2, 0, 0 ] ]
			]
		};
	...
	TransformGizmoRotate = function () {
		...
		var CircleGeometry = function ( radius, facing, arc ) {
			...
			for ( var i = 0; i <= 64 * arc; ++ i ) {
				if ( facing == 'x' ) geometry.vertices.push( new Vector3( 0, Math.cos( i / 32 * Math.PI ), Math.sin( i / 32 * Math.PI ) ).multiplyScalar(radius) );
				if ( facing == 'y' ) geometry.vertices.push( new Vector3( Math.cos( i / 32 * Math.PI ), 0, Math.sin( i / 32 * Math.PI ) ).multiplyScalar(radius) );
				if ( facing == 'z' ) geometry.vertices.push( new Vector3( Math.sin( i / 32 * Math.PI ), Math.cos( i / 32 * Math.PI ), 0 ).multiplyScalar(radius) );
			}
		...
		this.pickerGizmos = {
			X: [
				[ new Mesh( new TorusGeometry( 1, 0.12, 4, 12, Math.PI ), new GizmoMaterial( { color: 0xff0000, opacity: 0.25 } ) ), [ 0, 0, 0 ], [ 0, -Math.PI / 2, -Math.PI / 2 ] ]
			],
			Y: [
				[ new Mesh( new TorusGeometry( 1, 0.12, 4, 12, Math.PI ), new GizmoMaterial( { color: 0x00ff00, opacity: 0.25 } ) ), [ 0, 0, 0 ], [ Math.PI / 2, 0, 0 ] ]
			],
			Z: [
				[ new Mesh( new TorusGeometry( 1, 0.12, 4, 12, Math.PI ), new GizmoMaterial( { color: 0x0000ff, opacity: 0.25 } ) ), [ 0, 0, 0 ], [ 0, 0, -Math.PI / 2 ] ]
			],
			E: [
				[ new Mesh( new TorusGeometry( 1.25, 0.12, 2, 24 ), new GizmoMaterial( { color: 0xffff00, opacity: 0.25 } ) ) ]
			],
			XYZE: [
				[ new Mesh( new Geometry() ) ]// TODO
			]
		};
	...
	TransformGizmoScale = function () {
		...
		this.handleGizmos = {
			...
			XYZ: [
				[ new Mesh( new BoxGeometry( 0.125, 0.125, 0.125 ), new GizmoMaterial( { color: 0xffffff, opacity: 0.25 } ) ) ]
			]
		};
		...
		this.pickerGizmos = {
			...
			XYZ: [
				[ new Mesh( new BoxGeometry( 0.4, 0.4, 0.4 ), new GizmoMaterial( { color: 0xffffff, opacity: 0.25 } ) ) ]
			]
		};
	...
	TransformControls = function ( camera, domElement ) {
		...
		var pointerVector = vec3.create();

		var point = vec3.create();
		var offset = vec3.create();

		var rotation = vec3.create();
		var offsetRotation = vec3.create();
		var scale = 1;
		...
		var tempQuaternion = new Quaternion();
		var unitX = createFrom(1, 0, 0);
		var unitY = createFrom(0, 1, 0);
		var unitZ = createFrom(0, 0, 1);
		...
		var oldPosition = vec3.create();
		var oldScale = vec3.create();
		var oldRotationMatrix = mat4.create();

		var parentRotationMatrix  = mat4.create();
		var parentScale = vec3.create();
