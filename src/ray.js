...
function generateRay(canvas, camera, x, y) {
  var n = viewportToClipping(e, canvas.width, canvas.height);
  var v = clippingtoView(n, camera);
  var g = viewToGlobal(v, camera);
}

function viewportToClipping(mouse, width, height) {
  var x = 2 * (mouse.x / width) - 1;
  var y = 1 - 2 * (mouse.y / height);
...
/**
 * Prepares bounding box for generic geometry
 */
